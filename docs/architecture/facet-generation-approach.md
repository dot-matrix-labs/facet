# Facet's Generation Architecture: The Split-Brain Approach

## 1. Partitioned Memory System
Facet employs a strict separation of concerns in its memory architecture, organizing data into distinct graph partitions:
...
Retrieval is not a static lookup but an active, agentic process. Facet utilizes an **Agentic RAG loop** built on **GraphRAG** principles:
...
Facet leverages a small, efficient **Local LLM** to perform critical pre-processing steps before any data leaves the device:
...
*   **Facet's Advantage**: By combining **Agentic Retrieval** (tool use) with **GraphRAG** principles on a **Local** partitioned graph, Facet achieves the depth of GraphRAG with the privacy and speed of edge computing, avoiding the massive cloud costs of Microsoft's approach.

## 7. Open Source Components for Facet
Instead of building from scratch, Facet leverages the high-performance Rust ecosystem:
